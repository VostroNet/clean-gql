...
import { cleanDocumentWithJTDMin, cleanObject, cleanDocumentWithJTDMinMeta } from '../../src/jtd-min';
import { JtdMinType, JtdType } from "@vostro/jtd-types";
...
test("clean document - basic test", () => {
  ...
  const newQuery = cleanDocumentWithJTDMin(query, rootSchema);
...
test("clean document with meta - basic arguments", () => {
  ...
  expect(op.variableTypes.arg1.t === JtdMinType.STRING);
  expect(op.variableTypes.arg2).toBeDefined();
  expect(op.variableTypes.arg2.rq).toBe(true);
  expect(op.variableTypes.arg2?.el).toBeDefined();
  // expect(op.variableTypes.arg2?.el?.t === rootSchema.def?.test1input1);
  expect(op.variableTypes.arg2?.el?.md?.n).toEqual(rootSchema.def?.test1input1?.md?.n);
});
